import React from 'react';

class Reset extends React.Component {
  constructor(props){
    super(props);
    this.state = {
      password:'',
      confpassword:'',
      _id:this.props.match.params._id
    }
    this.handlechange = this.handlechange.bind(this);
    this.handleclick = this.handleclick.bind(this);
  }

  handlechange(data, id) {
    this.setState({
      [id]: data.target.value
    });
  }

  handleclick(e){
    e.preventDefault();
    console.log(`handleclick ${this.state}`)
    if(this.state.password.length>6){
      if( this.state.password === this.state.confpassword){
        fetch("http://localhost:5000/user/newpassword", {
                method: 'POST',
                body: JSON.stringify(this.state), 
                headers:{'Accept': 'application/json',
                'Content-Type': 'application/json'
                }
              })
              .then(response => {
                console.log(`Success ${response.ok}`)
                if(response.ok){
                  alert('Password Updated!');
                  this.props.history.push('/login');
                }
                })
              .catch(error => console.error('error:',error));
      } else {
        alert('Passwords did not match!');
      }
    } else {
      alert('Password too short!');
    }

  }

  render() {
    return (
      <div>
        <div className='container'>
          <div className='content'>
            <div className='content_rgt'>
              <div className='register_sec'>
                <h1>Reset Password</h1>
                <ul>
                  <li>
                    <span>Enter New Password</span>
                    <input type='password' name='password' pattern=".{7,}" onChange={ (e) => this.handlechange(e, 'password') } placeholder='Enter your new password' />
                  </li>
                  <li>
                    <span>Confirm Password</span>
                    <input type='password' name='confpassword' pattern=".{7,}" onChange={ (e) => this.handlechange(e, 'confpassword') } placeholder='Enter your password again' />
                  </li>
                  <li>
                    <input type='submit' value='Submit' onClick={this.handleclick}/>
                  </li>
                </ul>
              </div>
            </div>
            <div className='content_lft'>
              <h1>Welcome from PPL!</h1>
              <p className='discrptn'>
                There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words which
                don't look even slightly believable. If you are going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in
                the middle of text.
              </p> <img src='/images/img_9.png' alt='' />
            </div>
          </div>
        </div>
        <div className='clear' />
      </div>
      );
  }
}


export default Reset;
